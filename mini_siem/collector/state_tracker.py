"""
State tracker for Windows Event Log processing.

This module keeps track of the "last processed record number" so the collector
can resume where it left off between runs.

It stores a small JSON file in the project-level `data/` folder.
"""
...
# Compute the project root so `data/` works regardless of the current working dir.
# `state_tracker.py` lives at: <root>/mini_siem/collector/state_tracker.py
...
def save_last_processed_record_number(last_record_number: int) -> None:
    ...
    # Store everything in a single JSON file with a stable key name.
...
def load_last_processed_record_number() -> int:
    """
    Load the last processed Windows Event Log record number from disk.

    Returns:
        The stored record number as an int.
        Defaults to `0` if the state file (or key) does not exist or is invalid.
    """

    # Default value: if the file doesn't exist yet, start from 0.
    ...
    try:
        ...
        # If the file can't be read or parsed, fall back to default.
        return 0
